    ...
    while True:
        try:
            if attemp == 3:
                return None
            if data is None:
                res = ses.get(url=url, timeout=1000)
            elif data == "":
                res = ses.post(url=url, timeout=1000)
            else:
                res = ses.post(url=url, data=data, timeout=1000)
            return res
        except (
...
class Referral:
    ...
    def start(self, referral_code):
        ...
        res = http(ses=self.ses, url=verify_referral_code_url, data=json.dumps(data))
        if res is None:
            return None
        ...
        if res is None:
            return None
        if not "registered wallet address successfully" in res.json().get("message"):
            log("failed register !")
            log(f"http response : {res.text} ")
            return None
